...
const PaymentBookHotel = () => {
  ...
  useEffect(() => {
    document.title = "Thanh toán đặt phòng";
  ...
  const handlePayment = async () => {
    try {
      const response = await HotelService.payment(
        hotelCode,
        bookingCode,
        {
      ...
      if (response.status === 200) {
        alert(
          "Thanh toán thành công, bạn sẽ được chuyển hướng về trang xem chi tiết đơn đặt phòng"
        );
        // navigate(`/`);
        navigate(`/book-hotel-detail/${hotelCode}/${bookingCode}`);
      }
    } catch (error) {
...
export default PaymentBookHotel;
